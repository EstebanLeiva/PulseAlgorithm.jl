...
function create_node!(graph::Graph, name::String, index::Int)
    n = index
    node = Node(name, Dict{Int, Link}())
    graph.nodes[n] = node
    graph.name_to_index[name] = n
...
"""
    find_node(graph::Graph, name::String)

Find the node index given the node name.
"""
function find_node(graph::Graph, name::String)
    return get(graph.name_to_index, name, -1)
end

"""
    find_or_add_node!(graph::Graph, name::String)

Find the node index given the node name. If the node does not exist, create a new node.
"""
function find_or_add_node!(graph::Graph, name::String)
    n = find_node(graph, name)
    if n < 0
...
function find_or_add_node!(graph::Graph, name::String, index::Int)
    n = find_node(graph, name)
    if n < 0
        n = create_node!(graph, name, index)
    end
    return n
end
...
"""
function add_link!(graph::Graph, src_name::String, dst_name::String, deterministic::Dict{String, Float64}, random::Dict{String, Dict{String, Float64}})
    u = find_or_add_node!(graph, src_name)
    v = find_or_add_node!(graph, dst_name)
    graph.nodes[u].links[v] = Link(deterministic, random)
end

function add_link!(graph::Graph, src_name::String, src_index::Int, dst_name::String, dst_index::Int, deterministic::Dict{String, Float64}, random::Dict{String, Dict{String, Float64}})
    u = find_or_add_node!(graph, src_name, src_index)
    v = find_or_add_node!(graph, dst_name, dst_index)
    graph.nodes[u].links[v] = Link(deterministic, random)
end
"""
...
"""
function reverse_graph(graph::Graph) #TODO: improve the efficiency of this function
    new_graph = Graph(Dict{Int, Node}(), Dict{String, Int}(), Dict{Tuple{String, Tuple{Int, Int, Int, Int}}, Float64}())
    for (u, node) in graph.nodes
        create_node!(new_graph, node.name, u)
    end
    for (u, node) in graph.nodes
        for (v, link) in node.links
            add_link!(new_graph, graph.nodes[v].name, v, graph.nodes[u].name, u, link.deterministic, link.random)
        end
    end
    return new_graph
end
